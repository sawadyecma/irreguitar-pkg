...
describe("diff", () => {
  test("A4-G4=+2", () => {
    const got = new AbsnoteImpl(Absnotes.G4).diff(new AbsnoteImpl(Absnotes.A4));
    expect(got).toBe(+2);
  });

  test("G4-A4=-2", () => {
    const got = new AbsnoteImpl(Absnotes.A4).diff(new AbsnoteImpl(Absnotes.G4));
    expect(got).toBe(-2);
  });

  test("C2-C1=+12", () => {
    const got = new AbsnoteImpl(Absnotes.C1).diff(new AbsnoteImpl(Absnotes.C2));
    expect(got).toBe(+12);
  });
});
